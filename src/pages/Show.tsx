...
const Show: FC = () => {
  ...
  const { data: stories, loading } = useFetch<number[]>('https://hacker-news.firebaseio.com/v0/showstories.json?print=pretty')
            ...
            dataContent && (
              <>
                ...
                <div className="p-4 pb-0">
                  <p className="text-gray-700 text-sm">Link : <a href={dataContent?.url} target="_blank">{dataContent?.url}</a></p>
                  {dataContent?.text &&
                    <p dangerouslySetInnerHTML={{ __html: dataContent.text }} className='whitespace-normal font-sans text-gray-700 leading-7 mt-4'></p>
                  }
                </div>
              </>
            )
